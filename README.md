...
Same design language as `valdium.xyz` (Plus Jakarta + DM Mono + dashed rails + dark theme). Talks to any Valdium node over Ethereum-style JSON-RPC. Default RPC is `https://testnet.valdium.xyz`.
...
```
   Browser ── HTTPS ── nginx ──► Next.js explorer  (:3001)
                                       ...
                                       │  /api/indexer/* proxy
                                       ▼
                          valdium-indexer service  (:3002, 127.0.0.1)
                                       │
                                       ▼
                                  SQLite file
```
...
|---|---|---|
| `NEXT_PUBLIC_RPC_URL` | `https://testnet.valdium.com` | JSON-RPC endpoint. Must be reachable from the browser (CORS-friendly). |
...
| Variable | Default | What it does |
|---|---|---|
| `RPC_URL` | `https://testnet.valdium.com` | Where the indexer reads blocks from. |
...
```bash
sudo mkdir -p /opt/valdium-explorer
sudo chown $USER /opt/valdium-explorer
cd /opt/valdium-explorer
git clone <this-repo> .
...
sudo mkdir -p /var/lib/valdium-indexer
sudo chown -R www-data:www-data /var/lib/valdium-indexer
```

Set `DB_PATH=/var/lib/valdium-indexer/index.db` in `indexer/.env` so the database lives outside the source checkout (survives `git pull` and isn't owned by git).
...
```ini
# /etc/systemd/system/valdium-explorer.service
[Unit]
Description=Valdium Block Explorer
After=network.target valdium-indexer.service

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/valdium-explorer
...
```ini
# /etc/systemd/system/valdium-indexer.service
[Unit]
Description=Valdium Chain Indexer
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/valdium-explorer/indexer
ExecStart=/usr/bin/npm start
Restart=on-failure
Environment=NODE_ENV=production
...
```bash
sudo chown -R www-data:www-data /opt/valdium-explorer
sudo systemctl daemon-reload
sudo systemctl enable --now valdium-indexer
sudo systemctl enable --now valdium-explorer
sudo systemctl status valdium-indexer valdium-explorer
```

Watch the indexer backfill:

```bash
sudo journalctl -fu valdium-indexer
...
```nginx
# /etc/nginx/sites-available/explorer.valdium.com
server {
  listen 80;
  server_name explorer.valdium.com;
...
```bash
sudo ln -s /etc/nginx/sites-available/explorer.valdium.com /etc/nginx/sites-enabled/
sudo nginx -t
...
- Point `explorer.valdium.com` A-record at the box's IPv4.
- After DNS propagates:

```bash
sudo apt install -y certbot python3-certbot-nginx
sudo certbot --nginx -d explorer.valdium.com
```
...
sudo systemctl restart valdium-indexer valdium-explorer
```

The indexer's SQLite file lives outside the source tree (`/var/lib/valdium-indexer/index.db`), so it survives `git pull` and gradually grows over time. To force a full re-index, stop the service, delete the file, and restart. backfill from block 0 will run automatically.
...
- **Reorg handling**. Valdium's BFT finality is immediate, so this isn't an issue today. If consensus ever pivots, add a parent-hash sanity check before each insert.
...
│   └── validators.js
├── styles/
│   └── globals.css      # Tailwind v4 + design tokens (matches valdium.com)
