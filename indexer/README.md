...
Background indexer that scans the Valdium chain via JSON-RPC and writes a per-address transaction history into a local SQLite database. Exposes a small HTTP API on `127.0.0.1:3002` for the Next.js explorer to query.
...
- **No reorg handling.** Valdium's BFT finality means committed blocks don't get rewritten in practice. If consensus ever changes, add a parent-hash check before each insert.
